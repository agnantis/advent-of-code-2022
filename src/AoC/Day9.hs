module AoC.Day9 (mainDay9) where
...
followMultiHead :: Head -> [Tail] -> (Tail, [Tail]) -- Returns the last element and the whole list
                                                    -- to avoid re-reverting the list
followMultiHead h tls =
  let newTails = foldl' (\xs v -> (followHead (head xs) v):xs) [h] tls
  in (head newTails, tail $ reverse newTails) -- the list includes head as well at the head
...
simulateMulti :: Int -> Input -> Output
simulateMulti no = length . fst . foldl' folder (S.empty, (initPosition, replicate no initPosition))
 where
  folder :: (Set Coords, (Head, [Tail])) -> Move -> (Set Coords, (Head, [Tail]))
  folder (visited, (hd, tls)) mv =
    let headNewPos = moveTo hd mv
        (tailNewPos, newTails) = followMultiHead headNewPos tls
    in (S.insert tailNewPos visited, (headNewPos, newTails))
---

fstStar :: Input -> Output
fstStar = simulateMulti 1 -- simulate

sndStar :: Input -> Output
sndStar = simulateMulti 9
