...
type Input = [Command]
data Output = Fst { unFst :: Int} | Snd { unSnd :: String }
...
traverse' input =
 ...
 where
  ...
  folder (!xV, !acc) (msk, indx, Add v) = if msk then (xV+v, acc+(indx*(xV))) else (xV+v, acc)
  folder (!xV, !acc) (msk, indx, _) = if msk then (xV, acc+(indx*xV)) else (xV, acc)

traverse2 :: Input -> String
traverse2 input = unlines . chunksOf 40 . reverse . snd $ foldl' folder (initialValue, initialDrawing) (zip [0..] input)
 where
   nextXv :: Int -> Command -> Int
   nextXv cV (Add v) = cV + v
   nextXv cV  _      = cV
   folder :: (Int, String) -> (Int, Command) -> (Int, String)
   folder (!xV, ln) (indx, cmd) =
    let col = indx `mod` 40
        chr = if col == xV-1 || col == xV || col == xV+1 then '#' else ' '
    in (nextXv xV cmd, chr:ln)

---

fstStar :: Input -> Output
fstStar = Fst . traverse'

sndStar :: Input -> Output
sndStar = Snd . traverse2
...
mainDay10 = do
  ...
  print . unFst . fstStar $ input
  putStrLn . unSnd . sndStar $ input
  -- putStrLn . show . ((unFst . fstStar) &&& (unSnd . sndStar)) $ input
